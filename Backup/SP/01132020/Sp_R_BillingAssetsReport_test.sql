...
DROP PROCEDURE [dbo].Sp_R_BillingAssetsReport_test
GO
...
CREATE Procedure [dbo].[Sp_R_BillingAssetsReport_test]
as
/*

		EXEC [Sp_R_BillingAssetsReport_test]
...
--	 ---*************************************************************INSERT INTO @BADATA TABLE ****************************************************************************	
				...
				select 
						...
						, Sum(case when periodstartdate>=D.BAStartdate and PeriodEndDate<=D.BAEndDate then BillableAmount else 0 end) as Billablevalue
						, null as 'AssetsReporting'
						, null as 'TotalAssets'
						, SUM(CASE WHEN periodstartdate>=Dateadd(Month, -12, D.BASTARTDATE) and PeriodEndDate<=D.BASTARTDATE THEN (BILLABLEAMOUNT)/12 else 0  END) as Average
...
--		-----************************************************************* FINAL SELECT QUERY OF BOTH THE  TABLES @BADATA AND @gLDATA WITH FULL JOIN ****************************************************************************	
			...
			--	insert into Dynasty_DW..BillingFirm_Fact
				select 
						...
						, SUM(BA.ASSETADVISORY) as 'AssetAdvisory'
						, null as 'AssetsReporting'
						, null as 'TotalAssets'
