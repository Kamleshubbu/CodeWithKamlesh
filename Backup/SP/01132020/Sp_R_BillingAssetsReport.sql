...
--	 ---*************************************************************INSERT INTO @BADATA TABLE ****************************************************************************	
				...
				select 
						...
						, Sum(case when periodstartdate>=D.BAStartdate and PeriodEndDate<=D.BAEndDate then BillableAmount else 0 end) 
						, null as 'AssetsReporting'
						, null as 'TotalAssets'
						, SUM(CASE WHEN periodstartdate>=Dateadd(Month, -12, D.BASTARTDATE) and PeriodEndDate<=D.BASTARTDATE THEN (BILLABLEAMOUNT)/12 else 0  END) 
