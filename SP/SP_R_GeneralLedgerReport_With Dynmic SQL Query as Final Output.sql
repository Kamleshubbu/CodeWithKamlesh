...
---=============================================================================================================================
			...
			(
				...
				QuaterEndAmount Numeric(26,2) Null		,
				LastQuaterEndAmount Numeric(26,2) Null	,
				YearEndAmount Numeric(26,2) Null		,
				[Orderby] Float NUll
			)
	...
	----		-----===========================================================Used For formatting The Dataset =========================================
			...
			--,  Boldflg
			----, SectionTypeName
		 --   , SectionSubTypeName as 'Section Sub Type Name'
			--, ('$' + Replace(CONVERT(varchar, CAST(Round(QuaterEndAmount,2) AS money), 1),'',' ') )  as 'Quater End Amount'
			--, ('$' + Replace(CONVERT(varchar, CAST(Round(LastQuaterEndAmount,2) AS money), 1),'',' ') ) as 'Last Quater End Amount'
			--, ('$' + Replace(CONVERT(varchar, CAST(Round(YearEndAmount,2) AS money), 1),'',' ') ) as 'Year End Amount'
			--, Orderby 
			--from #Reportdata
			----Where QuaterEndAmount is not null and LastQuaterEndAmount is not null and YearEndAmount is not null
			--order BY [Orderby] 
			...
			Set @Sql =   
						 ...
						 Select     TopUnderlineFLg
									...
									,  TotalFlg	
									,  Boldflg
									, SectionSubtypeName AS Section 
									, ''$'' + (CONVERT(varchar, CAST(Round(QuaterEndAmount,2) AS money), 1)) '+' as '+'   [' +'Actual'+' '+ CONVERT(VARCHAR(20) , @QtrStartDT , 101)+ '                    To              ' + '   '+ CONVERT(VARCHAR , @AsofDate , 101) +' ]  
									, ''$'' + (CONVERT(varchar, CAST(Round(LastQuaterEndAmount,2) AS money), 1)) '+' as '+'   [' +'Actual' +' '+ CONVERT(VARCHAR(20) , @LastQtrStartDT , 101)+ '                    To              ' + '   '+ CONVERT(VARCHAR , @LastQtrEndDT  , 101) +' ]  
									, ''$'' + (CONVERT(varchar, CAST(Round(YearEndAmount ,2) AS money), 1)) '+' as '+'   [' +'Actual'+' '+ CONVERT(VARCHAR(20) , @YTDStartDT  , 101)+ '                    To              ' + '   '+ CONVERT(VARCHAR , @AsofDate , 101) +' ]  
									, orderby
