...
--	 ---*************************************************************INSERT INTO @BADATA TABLE ****************************************************************************	
				...
				select 
						...
						, Sum(case when periodstartdate>=D.BAStartdate and PeriodEndDate<=D.BAEndDate and BillTypeName not in ('REBILL','REBATE','Adjustment') and ProductTypeName not in ('REPORTING ONLY','REPORTING ONLY V2')  then BillableAmount else 0 end) as 'AssetAdvisory'
						, Sum(case when periodstartdate>=D.BAStartdate and PeriodEndDate<=D.BAEndDate and BillTypeName not in ('REBILL','REBATE','Adjustment') and ProductTypeName in ('REPORTING ONLY','REPORTING ONLY V2')  then BillableAmount else 0 end) as 'AssetsReporting'
						, null as 'TotalAssets'
						, SUM(CASE WHEN periodstartdate>=Dateadd(Month, -12, D.BASTARTDATE) and PeriodEndDate<=D.BASTARTDATE and BillTypeName not in ('REBILL','REBATE','Adjustment') and ProductTypeName not in ('REPORTING ONLY','REPORTING ONLY V2') THEN (BILLABLEAMOUNT)/12 else 0  END) as Average
