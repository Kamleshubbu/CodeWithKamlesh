USE Dynasty_DW
GO
IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[billingfirm_New_fact]') AND OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW dbo.vw_BillingFirm_New

GO
/* 
		
			select * from vw_BillingFirm_New


*/
create view [dbo].[vw_BillingFirm_New]
as 
	select BL.[CompanyName]
	  ...
	  from  Dynasty_DW.dbo.BillingFirm_New_Fact BL
